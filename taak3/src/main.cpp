...
#define MY_ID         (MY_GROUP_ID + 500)
#define MY_SIGN       ("TAAK3")
//
//============</identities>============
...
//============<taak3>============
#define TAAK3_ONOFF_KEY         1000
#define TAAK3_LEFT_EYE_KEY      1001
#define TAAK3_RIGHT_EYE_KEY     1002
//============</taak3>===========
...
//-*-*-*-*-*-*-*-*-*-*-*-*-
void taak_onoff_on() {
  Serial.println("taak_onoff_on!");
  digitalWrite(D5, HIGH);
}
Task taak_onoff_on_task(0, TASK_ONCE, &taak_onoff_on);

void taak_onoff_off() {
  Serial.println("taak_onoff_off!");
  digitalWrite(D5, LOW);
}
Task taak_onoff_off_task(0, TASK_ONCE, &taak_onoff_off);

void taak_left_eye_on() {
  Serial.println("taak_left_eye_on!");
  digitalWrite(D6, HIGH);
}
Task taak_left_eye_on_task(0, TASK_ONCE, &taak_left_eye_on);

void taak_left_eye_off() {
  Serial.println("taak_left_eye_off!");
  digitalWrite(D6, LOW);
}
Task taak_left_eye_off_task(0, TASK_ONCE, &taak_left_eye_off);

void taak_right_eye_on() {
  Serial.println("taak_right_eye_on!");
  digitalWrite(D7, HIGH);
}
Task taak_right_eye_on_task(0, TASK_ONCE, &taak_right_eye_on);

void taak_right_eye_off() {
  Serial.println("taak_right_eye_off!");
  digitalWrite(D7, LOW);
}
Task taak_right_eye_off_task(0, TASK_ONCE, &taak_right_eye_off);
//*-*-*-*-*-*-*-*-*-*-*-*-*
...
void onNoteHandler(Note & n) {
  ...
  if (n.id == MY_GROUP_ID || n.id == MY_ID) {
    ...
    if (n.velocity == 0) {
      if (n.onoff == 1) {
        if (n.pitch == TAAK3_ONOFF_KEY) taak_onoff_on_task.restartDelayed(10);
        else if (n.pitch == TAAK3_LEFT_EYE_KEY) taak_left_eye_on_task.restartDelayed(10);
        else if (n.pitch == TAAK3_RIGHT_EYE_KEY) taak_right_eye_on_task.restartDelayed(10);
      } else {
        if (n.pitch == TAAK3_ONOFF_KEY) taak_onoff_off_task.restartDelayed(10);
        else if (n.pitch == TAAK3_LEFT_EYE_KEY) taak_left_eye_off_task.restartDelayed(10);
        else if (n.pitch == TAAK3_RIGHT_EYE_KEY) taak_right_eye_off_task.restartDelayed(10);
      }
    }
    //taak_hit
    else {
      if (n.pitch == TAAK3_ONOFF_KEY) {
        taak_onoff_on_task.restartDelayed(10);
        taak_onoff_off_task.restartDelayed(10 + n.velocity * 2);
      }
      else if (n.pitch == TAAK3_LEFT_EYE_KEY) {
        taak_left_eye_on_task.restartDelayed(10);
        taak_left_eye_off_task.restartDelayed(10 + n.velocity * 2);
      }
      else if (n.pitch == TAAK3_RIGHT_EYE_KEY) {
        taak_right_eye_on_task.restartDelayed(10);
        taak_right_eye_off_task.restartDelayed(10 + n.velocity * 2);
      }
    }
  }
}
...
void setup() {
  ...
  //tasks
  runner.addTask(taak_onoff_on_task);
  runner.addTask(taak_onoff_off_task);
  runner.addTask(taak_left_eye_on_task);
  runner.addTask(taak_left_eye_off_task);
  runner.addTask(taak_right_eye_off_task);
  runner.addTask(taak_right_eye_on_task);
}
